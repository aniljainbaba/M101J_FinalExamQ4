...
public class BlogPostDAO {
    ...
    public void likePost(final String permalink, final int ordinal) {
		...
		DBObject comment = (DBObject) comments.get(ordinal);
		Integer numLikes = (Integer) comment.get("num_likes") + 1;

		WriteResult result = postsCollection.update(
						new BasicDBObject("permalink", permalink),
						new BasicDBObject("$set", new BasicDBObject("comments." + ordinal + ".num_likes", numLikes)),
						false,
						false
						);
